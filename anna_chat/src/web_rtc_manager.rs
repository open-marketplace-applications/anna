...
impl WebRTCManager {
    ...
    pub fn set_candidates(web_rtc_manager: Rc<RefCell<WebRTCManager>>, connection_string: &ConnectionString) {
    ...
    fn parse_base64_str_to_connection(str: &String) -> Result<ConnectionString, OfferError> {
        ...
        let connection_string: Result<ConnectionString, OfferError> = decoded_str.and_then(|a_str| {
            serde_json::from_str::<ConnectionString>(&a_str).map_err(|_| OfferError::SerializationError)
        });
    ...
    pub fn validate_answer(web_rtc_manager: Rc<RefCell<WebRTCManager>>, str: &String) -> Result<(), OfferError> {
        ...
        let remote_description_js_value: JsValue = JSON::parse(&connection_string.offer).expect("Expected valid json");
    ...
    pub fn validate_offer(web_rtc_manager: Rc<RefCell<WebRTCManager>>, str: &String) -> Result<(), OfferError> {
        let connection_string = WebRTCManager::parse_base64_str_to_connection(str);
        ...
        let set_local_description_function: SingleArgJsFn = Box::new(move |answer: JsValue| {
            ...
            web_rtc_manager_rc_clone.borrow_mut().offer = String::from(JSON::stringify(&answer).unwrap());
        });
    ...
    fn get_channel_status_change_closure(web_rtc_manager: Rc<RefCell<WebRTCManager>>) -> SingleArgClosure {
        let channel_status_change_closure = Closure::wrap(Box::new(move |_send_channel: JsValue| {
            let state = web_rtc_manager.borrow().data_channel.as_ref().unwrap().ready_state();
    ...
    fn get_on_data_closure(web_rtc_manager: Rc<RefCell<WebRTCManager>>) -> SingleArgClosure {
        let on_data = Closure::wrap(Box::new(move |arg: JsValue| {
            ...
            web_rtc_manager.borrow().parent_link.send_message(Msg::NewMessage(msg));
        }) as SingleArgJsFn);

        on_data
    }

    fn get_on_ice_candidate_closure(web_rtc_manager: Rc<RefCell<WebRTCManager>>) -> SingleArgClosure {
        let on_ice_candidate_closure = Closure::wrap(Box::new(move |ice_connection_event: JsValue| {
    ...
    fn get_on_ice_connection_state_change_closure(web_rtc_manager: Rc<RefCell<WebRTCManager>>) -> SingleArgClosure {
        let on_ice_connection_state_change = Closure::wrap(Box::new(move |_ice_connection_state_event: JsValue| {
    ...
    fn get_on_ice_gathering_state_change_closure(web_rtc_manager: Rc<RefCell<WebRTCManager>>) -> SingleArgClosure {
        let on_ice_gathering_state_change = Closure::wrap(Box::new(move |_ice_gathering_state: JsValue| {
    ...
    fn get_exception_handler(_web_rtc_manager: Rc<RefCell<WebRTCManager>>, _message: String) -> SingleArgClosure {
    ...
    fn set_data_channel(web_rtc_manager: Rc<RefCell<WebRTCManager>>, data_channel: RtcDataChannel) {
        ...
        web_rtc_manager.borrow_mut().channel_status_change_closure = Some(channel_status_change_closure);
    ...
    pub fn start_web_rtc(web_rtc_manager: Rc<RefCell<WebRTCManager>>) {
        let ice_servers = Array::new();
        {
            ...
            let _ = Reflect::set(&server_entry, &"urls".into(), &"stun:stun.l.google.com:19302".into());
        ...
        match state {
            State::Server(_connection_state) => {
                ...
                let data_channel: RtcDataChannel =
                    rtc_peer_connection.create_data_channel_with_data_channel_dict("sendChannel", &data_channel_init);
            ...
            State::Client(_connection_state) => {
                ...
                let on_data_channel_closure = Closure::wrap(Box::new(move |data_channel_event: JsValue| {
                    let data_channel_event = data_channel_event.unchecked_into::<RtcDataChannelEvent>();
                ...
                web_rtc_manager.borrow_mut().on_data_channel_closure = Some(on_data_channel_closure);
            }
        ...
        let on_ice_candidate_closure = WebRTCManager::get_on_ice_candidate_closure(web_rtc_manager.clone());
        ...
        rtc_peer_connection
            .set_oniceconnectionstatechange(Some(on_ice_connection_state_change_closure.as_ref().unchecked_ref()));

        rtc_peer_connection
            .set_onicegatheringstatechange(Some(on_ice_gathering_state_change_closure.as_ref().unchecked_ref()));
        ...
        web_rtc_manager.borrow_mut().on_ice_connection_state_change_closure =
            Some(on_ice_connection_state_change_closure);

        web_rtc_manager.borrow_mut().on_ice_gathering_state_change_closure =
            Some(on_ice_gathering_state_change_closure);
    }
}
